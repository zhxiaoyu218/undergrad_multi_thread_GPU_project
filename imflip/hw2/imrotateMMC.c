...
//    int row,col,h,v,c;
	...
	unsigned char BufferIm[BUFFER_SIZE];
	unsigned char BufferIm2[BUFFER_SIZE];

	
	double X2, Y2,newX2, newY2;
	double SRAYS2, CRAYS2;
	double  s1, s2;
	int NewRow2,NewCol2;
    ...
    tn *= ip.Vpixels/NumThreads;
			...
			h=ip.Hpixels/2;   v=ip.Vpixels/2;	// integer div
			hp3=ip.Hpixels*3;
    for(row=tn; row<tn+ip.Vpixels/NumThreads;row+=2)
	{
		col=0;
		cc=0.00;
		ss=0.00;
		...
		Y2=(double)v-(double)(row+1);
		SRAYS2=SRAS*Y2;    
		CRAYS2=CRAS*Y2;
		s1=CRAS*(double)h + SRAYS2;
		s2=SRAS*(double)h - CRAYS2;
		
		memcpy((void *) BufferIm, (void *) TheImage[row], (size_t) ip.Hbytes);
		memcpy((void *) BufferIm2, (void *) TheImage[row+1], (size_t) ip.Hbytes);
        ...
        while(col<hp3)
		{
			...
			if((NewCol>=0) && (NewRow>=0) && (NewCol<ip.Hpixels) && (NewRow<ip.Vpixels))
			{
				...
				NewCol2 = NewCol2 + NewCol2 + NewCol2;
				
				CopyImage[NewRow][NewCol]   = BufferIm[col];
				CopyImage[NewRow][NewCol+1] = BufferIm[col+1];
				CopyImage[NewRow][NewCol+2] = BufferIm[col+2];
				CopyImage[NewRow2][NewCol2]  = BufferIm2[col];
				CopyImage[NewRow2][NewCol2+1] = BufferIm2[col+1];
				CopyImage[NewRow2][NewCol2+2] = BufferIm2[col+2];
		...
		memcpy((void *)TheImage[row], (void *)BufferIm, (size_t)ip.Hbytes);
		memcpy((void *)TheImage[row+1], (void *)BufferIm2, (size_t)ip.Hbytes);
    }
    pthread_exit(NULL);
...
void *Rotate9(void* tid)
{
	...
	do{	
		...
		if(Last>=row)
		{
			NextRowToProcess+=2;
			col = *((int *)tid);
			*((int *)tid) = col+1; // One more row processed by this thread

		}
		pthread_mutex_unlock(&CounterMutex);
		if(Last<row) continue;
		if(row>=ip.Vpixels) break;
...
int main(int argc, char** argv)
{
	...
	printf("\nLaunching %d Pthread%s using function:  %s\n",NumThreads,NumThreads<=1?"":"s",FuncName);
	...
	for(a=0; a<REPS; a++)
	{
		...
		LastRowRead=-1;
		for(i=0; i<NumThreads; i++) ThParam[i]=0; // zero every thread counter	
		pthread_mutex_unlock(&CounterMutex);
