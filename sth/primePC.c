...
void *MTPrime(void* tid) 
{
	...
	ts *= NumOfSet / NumThreads;			// start index
...
void *MTPrime(void* tid) 
{
	int i,j,k,flag,temp,temp2;
	long LoopTime = ChunkSize/NumThreads;
	long ts = *((int *)tid);       	// My thread ID is stored here
	ts *= NumOfSet / NumThreads;			// start index
	long te = (ts + NumOfSet / NumThreads)/LoopTime - 1; 	// end index
	

	for(k = 0 ; k < LoopTime ; LoopTime++)
	{
		temp2 = te * (k+1);
		for (i = ts + (temp2 * k) ; i < temp2; i++)    
		{
			flag = 1;
			temp = (int)sqrt(numbers[i]);
			for (j = 2; j <= temp; j++)
...
int main(int argc, char** argv)
{
	...
	printf("\nThread:     %i\n\nChunkSize:  %i\n\nInputfile:  %s \n\nOutputfile: %s\n\n",NumThreads,ChunkSize,argv[1],argv[2]);
