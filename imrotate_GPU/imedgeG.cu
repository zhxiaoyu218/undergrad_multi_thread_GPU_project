...
__global__ void rotateG(unsigned char *GPU_i, unsigned char *GPU_o, int row_image, int col_image, double  RotAngle)
{
    ...
    while(col<Hbytes_image)
	{
		...
		if((NewCol>=0) && (NewRow>=0) && (NewCol<col_image) && (NewRow<row_image))
		{
				...
				GPU_o[NewRow*Hbytes_image + NewCol] = GPU_i[row*Hbytes_image+ col];
				GPU_o[NewRow*Hbytes_image + NewCol+1] = GPU_i[row*Hbytes_image + col+1];
				GPU_o[NewRow*Hbytes_image + NewCol+2] = GPU_i[row*Hbytes_image + col + 2];
...
cudaError_t launch_rotateG(Mat image, unsigned char *CPU_OutputArray, float* Runtimes, int row_image, int col_image, double RotAngle)
{
    ...
    TotalGPUSize = row_image*3*col_image*sizeof(unsigned char);
	...
	rotateG<<<numBlocks, threadsPerBlock>>>(GPU_idata, GPU_odata, row_image, col_image, RotAngle);
